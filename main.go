...
var (
	//go:embed payload/VolumeBoost.exe
	appPayload []byte

	//go:embed payload/VolumeBoost.ico
	iconPayload []byte
...
func appPath() string          { return filepath.Join(installDir(), "VolumeBoost.exe") }
func setupPath() string        { return filepath.Join(installDir(), "VolumeBoostSetup.exe") }
func iconPath() string         { return filepath.Join(installDir(), "VolumeBoost.ico") }
...
func markerPath() string       { return filepath.Join(dataDir(), "apo-installed-by-volumeboost") }
...
func managedConfigDir() string { return filepath.Join(apoConfigDir(), "VolumeBoost") }
...
func grantManagedConfigPermissions() error {
	...
	// Grant standard users Modify access only to VolumeBoost's private subfolder,
...
func integrateConfig() error {
	...
	backup := apoConfig() + ".volumeboost-backup"
...
func ensureGainFile() error {
	...
	content := []byte("# Managed by VolumeBoost\r\n# 100% = +0.00 dB\r\nPreamp: 0.00 dB\r\n")
...
func createShortcuts() error {
	...
	startMenu := filepath.Join(programData(), "Microsoft", "Windows", "Start Menu", "Programs", "VolumeBoost.lnk")
	...
	script := fmt.Sprintf("$w=New-Object -ComObject WScript.Shell;$d=$w.SpecialFolders('Desktop');$s=$w.CreateShortcut((Join-Path $d 'VolumeBoost.lnk'));$s.TargetPath='%s';$s.WorkingDirectory='%s';$s.IconLocation='%s,0';$s.Save()",
		psQuote(appPath()), psQuote(installDir()), psQuote(iconPath()))
	return runHidden("powershell.exe", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-Command", script)
}

func removeShortcuts() {
	_ = os.Remove(filepath.Join(programData(), "Microsoft", "Windows", "Start Menu", "Programs", "VolumeBoost.lnk"))
	script := "$w=New-Object -ComObject WScript.Shell;$p=Join-Path $w.SpecialFolders('Desktop') 'VolumeBoost.lnk';Remove-Item $p -Force -ErrorAction SilentlyContinue"
	_ = runHidden("powershell.exe", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-Command", script)
}

func addUninstallEntry() error {
	key := `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\VolumeBoost`
	values := [][]string{
		{"add", key, "/v", "DisplayName", "/t", "REG_SZ", "/d", appName, "/f"},
		{"add", key, "/v", "DisplayVersion", "/t", "REG_SZ", "/d", appVersion, "/f"},
		{"add", key, "/v", "Publisher", "/t", "REG_SZ", "/d", "VolumeBoost", "/f"},
...
func removeUninstallEntry() {
	_ = runHidden("reg.exe", "delete", `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\VolumeBoost`, "/f")
}

func stopRunningController() {
	setupLog("stopping any running VolumeBoost controller before update")
	cmd := exec.Command("taskkill.exe", "/IM", "VolumeBoost.exe", "/F")
	cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
	output, err := cmd.CombinedOutput()
	if text := strings.TrimSpace(string(output)); text != "" {
...
func installOrRepair(repair bool, launchApp bool) error {
	...
	if controllerInvokedRepair {
		setupLog("controller-invoked repair: skipping executable replacement while VolumeBoost.exe is running")
	} else {
...
func uninstall() error {
	...
	if messageBox("Remove VolumeBoost and its gain configuration?\n\nEqualizer APO will be left installed so other audio configurations are not damaged.", "Uninstall VolumeBoost", MB_YESNO|MB_ICONQUESTION) != IDYES {
		return nil
	}
	...
	cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
	_ = cmd.Start()
	messageBox("VolumeBoost was removed.\n\nEqualizer APO remains installed and can be removed separately from Windows Settings if you no longer use it.", "VolumeBoost", MB_OK|MB_ICONINFORMATION)
	return nil
}
...
func main() {
	...
	message := "VolumeBoost 1.7.0 is ready.\n\nThe emerald UI, dark mode, smooth controls, startup option, and close-to-tray setting are installed. Exiting VolumeBoost now resets APO gain to 100%."
	if repair {
		message = "VolumeBoost integration was repaired.\n\nThe live gain configuration was rebuilt and Windows Audio was restarted. Boost levels above 100% now use Windows 100% as their baseline."
	}
