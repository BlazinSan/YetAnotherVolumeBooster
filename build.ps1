...
try {
    ...
    go build -trimpath -ldflags '-H windowsgui -s -w' -o (Join-Path $Dist 'VolumeBoost.exe') .\app
    Copy-Item (Join-Path $Dist 'VolumeBoost.exe') (Join-Path $Payload 'VolumeBoost.exe') -Force
