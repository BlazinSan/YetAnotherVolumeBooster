...
func setupPreferredLogDir() string {
	if base := os.Getenv("ProgramData"); base != "" {
		return filepath.Join(base, "VolumeBoost", "logs")
	}
	if base := os.Getenv("LOCALAPPDATA"); base != "" {
		return filepath.Join(base, "VolumeBoost", "logs")
	}
	return filepath.Join(os.TempDir(), "VolumeBoost", "logs")
}

func initSetupLogging() string {
	candidates := []string{
		setupPreferredLogDir(),
		filepath.Join(os.Getenv("LOCALAPPDATA"), "VolumeBoost", "logs"),
		filepath.Join(os.TempDir(), "VolumeBoost", "logs"),
	}
